...
![图5 参数敏感性](../output/figures/fig_heatmap.png)
...
![图6 跨期套利杠杆压力测试](../output/figures/fig_calendar_leverage.png)

图 6：杠杆放大的对象是同一条跨期收益序列，而不是新的收益来源。IC 和 IM 的基础跨期策略年化收益均为负，杠杆后亏损和回撤同步扩大。虽然未触发穿仓，但保证金占用和回撤压力快速上升，说明该模块更适合作为研究补充，不适合依靠杠杆强行改造成主策略。
...
![图7 IC-IM相关性](../output/figures/fig_correlation.png)
...
![图8 Bootstrap分布](../output/figures/fig_bootstrap.png)
...
![图9 收益归因分解](../output/figures/fig_attribution.png)

图 9：归因图把方向暴露和残差补偿放在同一视角下比较。Always 策略的 β 接近或高于 1，说明收益中有相当部分来自权益多头敞口；残差年化约 9.4%–9.6%，对应基差收敛和展期补偿的混合回报。B<-1.5% 策略在多数时间空仓，β 明显下降，但收益厚度也同步下降。

日频归因 R² 恢复至合理水平（IC 0.68，IM 0.93），β 恢复至接近 1（IC Always β=0.96）——策略确有显著的权益方向暴露。Always 策略的残差年化约为 9.4%–9.6%，反映基差收敛和展期补偿的混合回报，不宜等同于独立 alpha。B<-1.5% 策略在大部分时间空仓，β 显著降低。

## 7. 补充分析

### 7.1 成本压力测试

![图10 成本压力测试](../output/figures/fig_cost_stress.png)
